...
export default async function AffiliatesPage({
  params,
  searchParams,
}: PageProps<"/[workspaceSlug]/affiliates">) {
            ...
            action={
              search || status ? (
                <Button asChild variant="secondary">
                  <Link href={`/${workspaceSlug}/affiliates`}>Clear filters</Link>
                </Button>
              ) : (
                <InviteAffiliateDialog
                  workspaceSlug={workspaceSlug}
              ...
              <TBody>
                {result.rows.map((row) => (
                  <TR key={`${row.affiliateId}-${row.participationId ?? "none"}`} interactive>
                    ...
                    <TD numeric>{formatMoney(row.revenueMinor, workspace.defaultCurrency)}</TD>
                    <TD numeric className="text-foreground">
                      {formatMoney(row.commissionMinor, workspace.defaultCurrency)}
                    </TD>
                    <TD numeric>{formatRate(row.customers, row.clicks)}</TD>
                  </TR>
                ))}
              </TBody>
            </Table>
            ...
            >
              ...
              <span className="flex gap-2">
                <Button asChild variant="secondary" size="sm" disabled={page <= 1}>
                  <Link href={`/${workspaceSlug}/affiliates?page=${page - 1}`}>Previous</Link>
                </Button>
                <Button asChild variant="secondary" size="sm" disabled={page >= pages}>
                  <Link href={`/${workspaceSlug}/affiliates?page=${page + 1}`}>Next</Link>
                </Button>
              </span>
            </nav>
          ) : null}
